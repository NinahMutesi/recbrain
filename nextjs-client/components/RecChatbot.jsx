...
function getSessionId() {
  ...
  if (!id) {
    id = uuidv4();
    localStorage.setItem('rec_session', id);
  }
  return id;
}
...
function getReportLink(content, edition) {
  ...
  if (c.includes('REC21')) return REPORT_LINKS.REC21;
  return null;
}
...
function CopyButton({ text }) {
  ...
  return (
    <button
      ...
      style={{
        background: 'none', border: '1px solid #D4EBD0', borderRadius: 6, padding: '3px 10px',
        cursor: 'pointer', fontSize: 11, color: copied ? '#5BAD4E' : '#aaa', marginTop: 6,
  ...
  return (
    <a
      ...
      style={{
        ...
        background: 'linear-gradient(135deg,#f0f9ee,#e0f5db)', border: '1px solid #b8ddb4',
        borderRadius: 6, padding: '3px 10px', cursor: 'pointer', fontSize: 11, color: '#2d6b27',
        textDecoration: 'none', marginTop: 6, marginLeft: 6, fontWeight: 600, transition: 'all 0.2s',
      }}
      onMouseEnter={(e) => {
        e.currentTarget.style.background = 'linear-gradient(135deg,#5BAD4E,#3d8b32)';
        e.currentTarget.style.color = '#fff';
        e.currentTarget.style.borderColor = '#3d8b32';
      }}
      onMouseLeave={(e) => {
        e.currentTarget.style.background = 'linear-gradient(135deg,#f0f9ee,#e0f5db)';
        e.currentTarget.style.color = '#2d6b27';
        e.currentTarget.style.borderColor = '#b8ddb4';
      }}
    >
      {link.label}
    </a>
...
// ── Main Component ──────────────────────────────────────────────────────────
export default function RecChatbot() {
      ...
      if (data.type === 'done') {
        ...
        setStreaming((prev) => {
          if (prev) {
            ...
            setMessages((m) => [...m, { id: uuidv4(), role: 'assistant', content: prev, sources: finalSources, showFollowUps: true }]);
          }
          return '';
        });
        sources.current = [];
    ...
    s.on('chat:history:response', ({ history }) => {
      if (history?.length > 0) {
        const loaded = history.map((h) => ({
          id: h.$id || uuidv4(), role: h.role, content: h.content, sources: [], showFollowUps: h.role === 'assistant',
        }));
  ...
  return (
    <div
      style={{ minHeight: '100vh', height: '100vh', display: 'flex', flexDirection: 'column', fontFamily: "'Inter',-apple-system,sans-serif", overflow: 'hidden', background: 'linear-gradient(160deg, #f0faf0 0%, #e8f5e9 30%, #f5fbf5 60%, #ffffff 100%)' }}
      onClick={() => menuOpen && setMenuOpen(false)}
    >
      {/* Header */}
      <header style={{ background: 'rgba(255,255,255,0.92)', backdropFilter: 'blur(12px)', boxShadow: '0 2px 16px rgba(91,173,78,0.12)', padding: '0 16px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', height: 64, flexShrink: 0, zIndex: 10, position: 'relative' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          <a href="https://www.memd.go.ug" target="_blank" rel="noreferrer" title="Ministry of Energy and Mineral Development">
            <img src={MEMD_LOGO} alt="MEMD" style={{ height: 44, width: 'auto', objectFit: 'contain' }} onError={(e) => { e.currentTarget.style.display = 'none'; }} />
          </a>
          ...
          <a href="https://nrep.ug" target="_blank" rel="noreferrer" title="National Renewable Energy Platform">
            <img src={NREP_LOGO} alt="NREP" style={{ height: 44, width: 'auto', objectFit: 'contain' }} />
          </a>
          <div style={{ width: 1, height: 36, background: '#D4EBD0' }} />
          <div>
            <div style={{ color: '#1a2e1a', fontWeight: 700, fontSize: 15, background: 'linear-gradient(135deg, #2d6b27, #5BAD4E)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>
              REC Assistant
            </div>
            <div style={{ color: '#5BAD4E', fontSize: 10, fontWeight: 500 }}>Renewable Energy Conference · REC22 – REC26</div>
          </div>
        </div>

        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          <a href="https://nrep.ug/rec/" target="_blank" rel="noreferrer" style={{ background: 'linear-gradient(135deg,#5BAD4E,#3d8b32)', color: '#fff', fontSize: 11, fontWeight: 700, textDecoration: 'none', padding: '5px 12px', borderRadius: 8, whiteSpace: 'nowrap', boxShadow: '0 2px 6px rgba(91,173,78,0.25)' }}>
            🎪 REC EXPO
          </a>
          <div style={{ display: 'flex', alignItems: 'center', gap: 5, fontSize: 11, color: connected ? '#5BAD4E' : '#e53935', fontWeight: 500 }}>
            <div style={{ width: 7, height: 7, borderRadius: '50%', background: connected ? '#5BAD4E' : '#e53935' }} />
            {connected ? 'Online' : 'Offline'}
          </div>
          <div style={{ position: 'relative' }}>
            <button onClick={(e) => { e.stopPropagation(); setMenuOpen(!menuOpen); }} style={{ background: 'none', border: '1px solid #D4EBD0', borderRadius: 8, padding: '5px 10px', cursor: 'pointer', fontSize: 18, color: '#5BAD4E', lineHeight: 1 }}>
              ⋮
            </button>
            {menuOpen && (
              <div style={{ position: 'absolute', top: 42, right: 0, background: '#fff', border: '1px solid #D4EBD0', borderRadius: 12, padding: 6, boxShadow: '0 8px 24px rgba(0,0,0,0.12)', zIndex: 200, minWidth: 190 }} onClick={(e) => e.stopPropagation()}>
                <button onClick={clearChat} style={{ display: 'block', width: '100%', background: 'none', border: 'none', padding: '9px 14px', textAlign: 'left', cursor: 'pointer', fontSize: 13, color: '#1a2e1a', borderRadius: 8 }}>🗑 New Chat</button>
                <div style={{ height: 1, background: '#E8F5E4', margin: '4px 8px' }} />
                <a href="https://nrep.ug/rec/" target="_blank" rel="noreferrer" style={{ display: 'block', padding: '9px 14px', fontSize: 13, color: '#1a2e1a', textDecoration: 'none', borderRadius: 8, fontWeight: 600 }}>🎪 REC EXPO 2026</a>
                <a href="https://www.memd.go.ug" target="_blank" rel="noreferrer" style={{ display: 'block', padding: '9px 14px', fontSize: 13, color: '#1a2e1a', textDecoration: 'none', borderRadius: 8 }}>🏛 MEMD Website</a>
                <a href="https://nrep.ug" target="_blank" rel="noreferrer" style={{ display: 'block', padding: '9px 14px', fontSize: 13, color: '#1a2e1a', textDecoration: 'none', borderRadius: 8 }}>🌐 NREP Website</a>
              </div>
            )}
          </div>
        </div>
        ...
        <div style={{ padding: '6px 16px', flexShrink: 0, background: 'rgba(255,255,255,0.7)', backdropFilter: 'blur(8px)', borderBottom: '1px solid rgba(212,235,208,0.5)' }}>
          <div style={{ maxWidth: 860, margin: '0 auto', display: 'flex', alignItems: 'center', gap: 8 }}>
            ...
            <div style={{ display: 'flex', gap: 6, overflowX: 'auto', paddingBottom: 2 }}>
              {QUICK_STARTS.map((q) => (
                <button key={q} onClick={() => send(q)} style={{ background: 'rgba(255,255,255,0.9)', border: '1.5px solid #D4EBD0', borderRadius: 20, padding: '4px 12px', fontSize: 12, color: '#2d5a27', cursor: 'pointer', fontWeight: 500, whiteSpace: 'nowrap', flexShrink: 0 }}>
                  {q}
                </button>
              ))}
            </div>
      ...
      {/* Input */}
      <div style={{ background: 'rgba(255,255,255,0.95)', backdropFilter: 'blur(12px)', borderTop: '1px solid rgba(212,235,208,0.8)', padding: '12px 16px', flexShrink: 0 }}>
        <div style={{ maxWidth: 860, margin: '0 auto', display: 'flex', gap: 8, alignItems: 'flex-end' }}>
          <textarea
            ...
            rows={1}
            style={{ flex: 1, background: 'rgba(247,249,247,0.9)', border: '1.5px solid #D4EBD0', borderRadius: 12, color: '#1a2e1a', padding: '11px 14px', fontSize: 14, resize: 'none', outline: 'none', fontFamily: 'inherit', minHeight: 44, maxHeight: 100 }}
          />
          <button onClick={() => send()} disabled={!input.trim() || thinking || isStreaming} style={{ background: input.trim() && !thinking && !isStreaming ? 'linear-gradient(135deg,#5BAD4E,#3d8b32)' : '#D4EBD0', border: 'none', borderRadius: 12, width: 44, height: 44, cursor: input.trim() ? 'pointer' : 'not-allowed', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 18, flexShrink: 0 }}>
            {thinking || isStreaming ? '⏳' : '➤'}
          </button>
        </div>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 10, marginTop: 8 }}>
          <span style={{ color: '#ccc', fontSize: 11 }}>
            Only answers REC Uganda questions ·{' '}
